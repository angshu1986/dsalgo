...
/**
 * Print all coins that make up a given value, assuming finite number of coins
 * are present with no repeating coins
 * 
 * @author angsh
 *
 */
public class PrintZeroOneCoinChange {
	...
	private static void printCoinChange(int[] coins, int val, List<List<Integer>> result, List<Integer> current,
		...
		for (int i = index; i < coins.length; i++) {
			if (coins[i] <= val) {
				current.add(coins[i]);
				printCoinChange(coins, val - coins[i], result, current, i+1);
